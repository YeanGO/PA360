...
def init_db() -> None:
    with db_session() as conn:
        ...
        # 1) 先去除重複（保留每組 teacher+target id 最大的那筆）
        conn.execute("""
        ...
        # 2) 建立唯一索引：同一對 (rater, target) 只能有一筆
        conn.execute("""
